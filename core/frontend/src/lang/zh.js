export default {
  ...
  user: {
    ...
    input_id_placeholder: '请输入用户ID (不支持中文)',
    source: '用户来源',
    choose_org: '请选择组织',
    reset_password: '重置密码',
    current_user: '当前用户',
  ...
  chart: {
    ...
    add_db_table: '添加数据库指标',
    add_api_table: '添加API指标',
    ...
    bolder: '加粗',
    change_ds: '更换指标',
    change_ds_tip: '提示：更换指标将导致字段发生变化，需重新制作视图',
    ...
    table_pivot_row: '数据行',
    field_error_tips: '该字段所对应的指标原始字段发生变更（包括维度、指标，字段类型，字段被删除等），建议重新编辑',
    mark_field_error: '指标变更，当前字段不存在，请重新选择',
    ...
    drill_dimension_tip: '钻取字段仅支持指标中的字段',
  ...
  dataset: {
    ...
    parse_filed: '解析字段',
    field_rename: '字段重命名',
    params_work: '仅编辑时生效：参数值仅在指标编辑时生效；全局生效：在指标查看、预览、以及用到指标的视图中均生效。',
    select_year: '选择年',
    ...
    tips: '提示',
    confirm_delete: '确认删除',
    confirm_delete_msg: '指标删除，会影响与其相关的自定义指标、关联指标、仪表板，确认删除？',
    delete_success: '删除成功',
    confirm: '确认',
    cancel: '取消',
    search: '搜索',
    ...
    edit_sql: '编辑 SQL 指标',
    showRow: '显示行',
    add_excel_table: '添加Excel指标',
    add_custom_table: '添加自定义指标',
    upload_file: '上传文件',
    detail: '详情',
    type: '类型',
    create_by: '创建者',
    ...
    invalid_dataset: 'Kettle未运行，无效指标',
    check_all: '全选',
    can_not_union_self: '被关联表不能与关联表相同',
    float: '小数',
    edit_custom_table: '编辑自定义指标',
    edit_field: '编辑字段',
    preview_100_data: '显示前100行数据',
    invalid_table_check: '非直连指标请先完成数据同步',
    ...
    m1: '将 ',
    m2: ' 移动到',
    char_can_not_more_50: '指标名称不能超过50个字符',
    task_add_title: '添加任务',
    ...
    task: {
      ...
      excel_replace_msg: '可能会影响自定义指标、关联指标、模板等，确认替换？',
    ...
    inner_join: '内连接',
    full_join: '全连接',
    can_not_union_diff_datasource: '被关联指标必须与当前指标的数据源一致',
    operator: '操作',
    d_q_trans: '维度/指标转换',
    ...
    merge: '合并',
    no_merge: '不合并',
    merge_msg: '数据表中存在字段一致的情况，是否合并到一个指标中?',
    merge_title: '合并数据',
    ...
    sync_success: '同步成功',
    sync_success_1: '同步成功，请对当前指标重新执行数据同步操作',
    row_permission: {
      type: '类型',
      name: '名称',
      condition: '条件',
    ...
    union_repeat: '当前指标已被关联，请勿重复关联',
    preview_result: '预览结果',
    sql_ds_union_error: '直连模式下SQL指标，不支持关联',
    api_data: 'API指标',
    copy: '复制',
    sync_log: '同步日志',
    ...
    calc_tips: {
      ...
      tip6: '使用指标对应数据库类型所支持的函数，语法同对应数据库',
      tip7: '如日期格式化：MySQL使用DATE_FORMAT(date,format)；Oracle使用TO_DATE(X,[,fmt])',
      tip8: '非直连模式指标，使用Doris数据库函数，可参考Doris官网'
    }
  },
  driver: {
    driver: '驱动',
  ...
  datasource: {
    ...
    edit_datasource_msg: '修改数据源信息，可能会导致该数据源下的指标不可用，确认修改？',
    ...
    delete_this_dataset: '确定删除该指标吗？',
    cannot_be_deleted_dataset: '该指标存在如下血缘关系，删除会造成相关仪表板的视图失效，确定删除？',
  ...
  panel: {
    ...
    template_view_tips: '当前是模板视图,请点击更换指标',
  ...
  webmsg: {
    ...
    i18n_msg_type_dataset_sync: '指标同步',
    i18n_msg_type_dataset_sync_success: '指标同步成功',
    i18n_msg_type_dataset_sync_failed: '指标同步失败',
  ...
  app_template: {
    ...
    panel_name: '仪表板名称',
    dataset_group_position: '指标分组位置',
    dataset_group_name: '指标分组名称',
